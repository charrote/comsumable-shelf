...
export default function InboundScreen() {
  ...
  const startReceipt = useCallback(async () => {
    ...
    setIsLoading(true)
    try {
      const receipt = await createReceiptApi({ operator })
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>扫码入库</Text>
      <Text style={styles.receiptInfo}>入库单 #{receiptId}</Text>
...
const styles = StyleSheet.create({
  ...
  button: { backgroundColor: '#52c41a', padding: 16, borderRadius: 8, alignItems: 'center', marginBottom: 16 },
