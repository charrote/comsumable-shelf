...
用户浏览器 → http://192.168.1.X (设备局域网 IP)
                    │
          ┌───────┴───────┐
          │    Nginx      │  ← 统一入口, 监听 80/443
          │               │
          │  /api/*       │ → 反向代理到 8080 (FastAPI)
          │  / (其他)     │ → 返回前端静态文件 (Vite 打包产物)
          └───────┬───────┘
                  │
          ┌───────┴───────┐
          │  FastAPI      │ → /api/* 请求处理
...
## 请求流向：
http://IP:80/          → 前端页面 (Nginx serve dist)
http://IP:80/api/*     → Nginx 转发到 backend:8080/api/*
http://IP:80/ws/*      → WebSocket 转发到 backend:8080/ws/*
http://IP:8080/api/*   → 外部设备直接调后端（PDA、点料机）
