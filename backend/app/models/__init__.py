...
class ShelfSlot(Base):
    ...
    board_address = Column(Integer, default=0, comment="板地址")
    slot_on_board = Column(Integer, nullable=False, comment="板上编号")
    global_index = Column(Integer, default=0, comment="全局索引")
    modbus_tcp_id = Column(Integer, default=0, comment="Modbus TCP ID")
    modbus_coil_base = Column(Integer, default=0, comment="Modbus 线圈基地址")
    code = Column(String, comment="储位编号（如 A-01）")
    name = Column(String, comment="储位名称")
    cell_id = Column(String(32), unique=True, nullable=True, comment="自动生成: UPPER(shelf_code + code)")
